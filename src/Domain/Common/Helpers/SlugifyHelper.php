...
final class SlugifyHelper
{
    ...
    {
        /** @var string $stringReplace */
        $stringReplace = preg_replace(
            ['/[^a-zA-Z0-9 \'-]/', '/[ -\']+/', '/^-|-$/'],
            ['', '-', ''],
            self::removeAccent($text)
        );

        return mb_strtolower($stringReplace);
    }
     ...
     * @return mixed
     */
    private static function removeAccent(string $text)
    {
